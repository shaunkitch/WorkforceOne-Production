...
import { Loader2, Palette, Shield, HardDrive, LayoutGrid } from "lucide-react";
...
export default function SettingsClient({ orgId, org, usage }: SettingsClientProps) {
    const [saving, startTransition] = useTransition();

    const [name, setName] = useState(org?.name || "");
    const [brandColor, setBrandColor] = useState(org?.brand_color || "#09090b");
    const [logoUrl, setLogoUrl] = useState(org?.logo_url || "");
    const [appLogoUrl, setAppLogoUrl] = useState(org?.app_logo_url || "");
    const [faviconUrl, setFaviconUrl] = useState(org?.favicon_url || "");
    const [currency, setCurrency] = useState(org?.currency || "USD");

    // Feature Flags
    const [features, setFeatures] = useState({
        payroll: org?.features?.payroll || false,
        crm: org?.features?.crm || false,
        operations: org?.features?.operations || false,
        automations: org?.features?.automations || false,
        security: org?.features?.security || false,
        security_manual_check: org?.features?.security_manual_check || false,
    });
