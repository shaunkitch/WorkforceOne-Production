...
const type: ElementsType = "LicenseField";

const extraAttributes = {
    label: "License Scanner",
    helperText: "Scan the back of a driver's license",
    required: false,
};
...
export const LicenseFieldFormElement: FormElement = {
    type,
    construct: (id: string) => ({
        id,
        type,
...
function FormComponent({
    ...
    const { result } = useZxing({
        onDecodeResult(result) {
            ...
            setIsScanning(false);
            if (submitValue) {
                const valid = LicenseFieldFormElement.validate(element, text);
                setError(!valid);
                ...
                {isScanning ? (
                    <div className="relative aspect-video w-full overflow-hidden rounded-md bg-black">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="w-64 h-40 border-2 border-primary/50 rounded-lg"></div>
                        </div>
                    </div>
                ) : (
